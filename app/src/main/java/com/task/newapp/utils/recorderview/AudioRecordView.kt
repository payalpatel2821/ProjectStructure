package com.task.newapp.utils.recorderview
...
import com.task.newapp.databinding.ViewAudioRecordBinding
import com.task.newapp.databinding.ViewSlidePanelBinding
import com.task.newapp.utils.recorderview.*
...
class AudioRecordView : FrameLayout {
    ...
    var binding: ViewAudioRecordBinding =
        ViewAudioRecordBinding.inflate(LayoutInflater.from(context), this)
    var _binding: ViewSlidePanelBinding = ViewSlidePanelBinding.bind(binding.root)

    private var isRecording = false
    private var upBeforeGrant = false

    @DrawableRes
    var micIcon: Int = R.drawable.ic_voice
        set(value) {
    ...
    @ColorInt
    var micTintColor: Int = ContextCompat.getColor(context, android.R.color.white)
        set(value) {
            if (value == field) return

            field = value
            binding.recordIb.setColorFilter(
    ...
    @DrawableRes
    var micActiveIcon: Int = R.drawable.ic_voice
        set(value) {
            if (value == field) return

            field = value
            binding.recordCircle.audioDrawable =
...
*/
    ...
    @ColorInt
    var circleColor: Int = ContextCompat.getColor(context, R.color.gray2)
        set(value) {
            if (value == field) return
    ...
    @ColorInt
    var blinkColor: Int = ContextCompat.getColor(context, R.color.red)
        set(value) {
            if (value == field) return
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        ...
        val ta = context.obtainStyledAttributes(attrs, R.styleable.AudioRecordViewInput)
        if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_icon)) {
            micIcon = ta.getResourceId(R.styleable.AudioRecordViewInput_mic_icon, 0)
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_tint_color)) {
            micTintColor = ta.getColor(R.styleable.AudioRecordViewInput_mic_tint_color, 0)
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_active_icon)) {
            micActiveIcon = ta.getResourceId(R.styleable.AudioRecordViewInput_mic_active_icon, 0)
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_hint_enable)) {
            micHintEnable = ta.getBoolean(R.styleable.AudioRecordViewInput_mic_hint_enable, true)
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_hint_text)) {
            ta.getString(R.styleable.AudioRecordViewInput_mic_hint_text)?.let {
                micHintText = it
            }
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_hint_color)) {
            micHintColor = ta.getColor(R.styleable.AudioRecordViewInput_mic_hint_color, 0)
        }
      /*  if (ta.hasValue(R.styleable.AudioRecordViewInput_mic_hint_bg)) {
            micHintBg = ta.getResourceId(R.styleable.AudioRecordViewInput_mic_hint_bg, 0)
        }*/
        if (ta.hasValue(R.styleable.AudioRecordViewInput_circle_color)) {
            circleColor = ta.getColor(R.styleable.AudioRecordViewInput_circle_color, 0)
        }
        /* if (ta.hasValue(R.styleable.AudioRecordViewInput_cancel_icon_color)) {
             cancelIconColor = ta.getColor(R.styleable.AudioRecordViewInput_cancel_icon_color, 0)
         }*/
        if (ta.hasValue(R.styleable.AudioRecordViewInput_blink_color)) {
            blinkColor = ta.getColor(R.styleable.AudioRecordViewInput_blink_color, 0)
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_slide_cancel_text)) {
            ta.getString(R.styleable.AudioRecordViewInput_slide_cancel_text)?.let {
                slideCancelText = it
            }
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_cancel_text)) {
            ta.getString(R.styleable.AudioRecordViewInput_cancel_text)?.let {
                cancelText = it
            }
        }
        if (ta.hasValue(R.styleable.AudioRecordViewInput_vibration_enable)) {
            vibrationEnable = ta.getBoolean(R.styleable.AudioRecordViewInput_vibration_enable, true)
        }
